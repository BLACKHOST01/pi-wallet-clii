...
function set() {
    ...
    const passphrase = prompt(chalk.yellowBright(`Account Passphrase/PrivateKey: `));
    if (StellarBase.StrKey.isValidEd25519PublicKey(passphrase)){
        config.account_Passphrase = passphrase
        var json = JSON.stringify(config);
        fs.writeFile(appDir + '/config.json', json, 'utf8', (err) => {
            if (err) throw err;
            console.log(chalk.yellowBright(`Account was set successfully!`))
